...
public class TesterExportController {
    ...
    public ResponseEntity<byte[]> exportTesterFullStatistics(
            @PathVariable String testerUsername,
            @AuthenticationPrincipal UserDetails userDetails) {

        log.info("Экспорт полной статистики тестировщика: {}", testerUsername);
        ...
        try {
            ...
            // Генерируем PDF
            byte[] pdfContent = generateTesterFullStatisticsPdf(tester, allAttempts);

            String filename = String.format("tester_%s_full_statistics_%s.pdf",
                    testerUsername,
            ...
            log.error("Ошибка при экспорте полной статистики тестировщика", e);
            return ResponseEntity.badRequest().build();
        }
    }

    // ---- Приватные методы генерации PDF ----

    private byte[] generateTesterFullStatisticsPdf(User tester, List<TesterAttemptDTO> attempts) {
